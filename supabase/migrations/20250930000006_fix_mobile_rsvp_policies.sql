-- Fix RLS policies for mobile RSVP - Allow anonymous guest confirmations
-- This migration specifically addresses mobile RSVP failures due to RLS restrictions
...
USING (
  ...
  OR
  -- Allow anonymous users to view by providing email in query context (for mobile)
  -- This is handled at application level with email matching
  (auth.uid() IS NULL)
);
...
USING (
  ...
  -- This allows guests to update their RSVP right after creating it
  (auth.uid() IS NULL)
);
...
-- Add policy to allow site creators to manage their sites
...
-- SELECT 'Mobile RSVP policies updated successfully' as status;