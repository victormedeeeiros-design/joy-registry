...
const PublicSiteContent = () => {
  ...
  const handleAddToCart = (siteProduct: SiteProduct) => {
    console.log('🛒 Adicionando ao carrinho:', siteProduct);
    
    // Para produtos do banco (sem .product) usa dados custom
    // Para fallback (com .product) usa combinação custom + product
    const product = siteProduct.product;
    
    const name = siteProduct.custom_name || (product?.name) || 'Produto sem nome';
    const price = siteProduct.custom_price || (product?.price) || 0;
    const image = siteProduct.custom_image_url || (product?.image_url) || '';
    
    if (!name || name === 'Produto sem nome') {
      console.warn('❌ Produto sem nome válido:', siteProduct);
      return;
    }
    ...
    console.log('✅ Item adicionado ao carrinho:', { name, price, image });
  };
          ...
          {products.length === 0 ? (
            ...
            (() => {
              ...
              const categorizedProducts = products.reduce((acc, siteProduct) => {
                ...
                let category = 'Eletrodomésticos'; // default
                
                if (product?.category) {
                  category = product.category;
                } else if (siteProduct.custom_name) {
                  // Inferir categoria pelo nome para produtos do banco
                  const name = siteProduct.custom_name.toLowerCase();
                  if (name.includes('micro') || name.includes('forno') || name.includes('fogão') || 
                      name.includes('geladeira') || name.includes('liquidificador') || name.includes('batedeira')) {
                    category = 'Eletrodomésticos';
                  } else if (name.includes('kit') || name.includes('pano') || name.includes('tapete') || name.includes('manual')) {
                    category = 'Brincadeiras';
                  } else if (name.includes('notebook') || name.includes('smartphone') || name.includes('fone')) {
                    category = 'Tecnologia';
                  }
                }
                    ...
                    <div key={category} className="space-y-4 sm:space-y-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
                        {categoryProducts.map((siteProduct, index) => {
                          ...
                          const name = siteProduct.custom_name || (product?.name) || 'Produto sem nome';
                          const price = siteProduct.custom_price || (product?.price) || 0;
                          const image = siteProduct.custom_image_url || (product?.image_url) || getProductImageFallback(siteProduct.product_id, name);
                          const description = siteProduct.custom_description || (product?.description) || '';
                          
                          // Pular produtos sem nome válido
                          if (!name || name === 'Produto sem nome') {
                            console.warn(`❌ Pulando produto sem nome:`, siteProduct);
                            return null;
                          }
                          
                          console.log(`✅ Produto ${index} será renderizado:`, { name, price, category });
